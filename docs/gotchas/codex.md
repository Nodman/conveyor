...
Rule: on this host `--strict-config` makes the canary/gate unrunnable (canary fails closed → exit 3 → advisory fallback, which is safe but disables the direct lane). Do NOT rely on `--strict-config` to catch `-c` typos while user configs carry MCP servers or other unknown fields — needs a design ruling (drop it and lean on the canary, or run codex against an isolated `CODEX_HOME`). Flagged, left in code (locked decision).
